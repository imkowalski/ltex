...
## PDF-to-editor navigation on Linux
...
To jump from a source line to the PDF manually, use:

```bash
ltex forward-search path/to/main.tex 42
```

The command resolves the project's PDF from `project.json` and sends the
position to the already-open Zathura window. Binding the current VS Code
cursor position to this command is supported by the VS Code task generated by
`ltex init --vscode`. Run Terminal → Run Task → Forward search in Zathura.

For a keyboard shortcut, add this to VS Code's user `keybindings.json`:

```json
{
  "key": "ctrl+alt+j",
  "command": "workbench.action.tasks.runTask",
  "args": "Forward search in Zathura"
}
```

With the cursor in a `.tex` file, pressing Ctrl+Alt+J sends the current file,
line, and column to Zathura.
...
still work, but ltex warns that inverse search is available only on Linux with
Zathura or Okular. VS Code and Codium are opened with `--reuse-window --goto`,
so an existing editor window is focused when possible.
...
ltex forward-search FILE LINE [COLUMN]
                            Jump from a source location to Zathura
